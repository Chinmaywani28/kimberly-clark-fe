...
const BigCard = ({ Data, chartData }) => {
  ...
  const dataValuesData = Data?.meter_reading
    ? convertDataValues(Data?.meter_reading.slice(1), power)
    : [];
  ...
  const convertedCriticalPower = criticalValue
    ? convertPower(criticalValue, power)
    : null;
  ...
  const powerValue = Data?.meter_reading?.length
    ? Data.meter_reading[Data.meter_reading.length - 1]
    : 0;
  ...
  function getGradient(ctx, data) {
    try {
      ...
      const getPosition = (value) => {
        return Math.min(
          1,
          Math.max(0, (value - minValue) / (maxValue - minValue || 1)),
        );
      };
    ...
    <div>
      <Card
        sx={{
          ...
          padding: 4,
          borderRadius: "20px",
          bgcolor: tempCheck?.bgColor,
          borderColor: tempCheck?.borderColor,
          borderWidth: 2,
        }}
      >
        {/* <div>afnkavn</div> */}
        <Box
          ref={ref}
          sx={{ color: tempCheck?.fontColor, position: "relative", bottom: 10 }}
        >
          <ToolTipBox
            title={`${"Device Name : " + (Data?.meter_name || "N/A")}`}
            arrow
          >
            <Typography
              sx={{
        ...
        >
          ...
          <Typography
            ...
            sx={{
              ...
              textOverflow: "ellipsis",
              width:
                Data?.gadget_type === "intermidiateCO2" ||
                Data?.gadget_type === "intermidiateCO"
                  ? 205
                  : null,
            }}
          >
        ...
        <Box
          sx={{
            display: "flex",
            justifyContent:
              Data?.gadget_type === "intermidiateValve"
                ? "center"
                : "space-evenly",
            alignItems: "center",
            position: "relative",
            bottom: Data?.gadget_type === "intermidiateValve" ? 85 : 44,
            gap: 2,
          }}
        >
          <Box
            sx={{
              display: "flex",
              flexDirection: "column",
              gap: Data?.meter_reading?.length ? 3 : 0,
              position: "relative",
              top: 35,
              left: Data?.gadget_type === "intermidiateValve" ? 60 : 0,
            }}
          >
                          ...
                          switch (Data?.gadget_type) {
                            case "intermidiateEnergy":
                              return [
                                <MenuItem
                                  key="wh"
                                  onClick={() => handleMenuItemClick("Wh")}
                                >
                                  Wh
                                </MenuItem>,
                                <MenuItem
                                  key="kwh"
                                  onClick={() => handleMenuItemClick("kWh")}
                                >
                                  kWh
                                </MenuItem>,
                              ];
                            ...
                            case "intermidiatePower":
                              return [
                                <MenuItem
                                  key="w"
                                  onClick={() => handleMenuItemClick("W")}
                                >
                                  W
                                </MenuItem>,
                                <MenuItem
                                  key="kw"
                                  onClick={() => handleMenuItemClick("kW")}
                                >
                                  kW
                                </MenuItem>,
                              ];
                            ...
                            case "intermidiateCO":
                              return [
                                <MenuItem
                                  key="co"
                                  onClick={() => handleMenuItemClick("ppm")}
                                >
                                  ppm
                                </MenuItem>,
                              ];
                            ...
                            case "intermidiatePressure":
                              return [
                                <MenuItem
                                  key="pressure"
                                  onClick={() => handleMenuItemClick("Pa")}
                                >
                                  Pa
                                </MenuItem>,
                              ];
                            case "intermidiateVibration":
                              return [
                                <MenuItem
                                  key="vibration"
                                  onClick={() => handleMenuItemClick("m/s2")}
                                >
                                  m/s2
                                </MenuItem>,
                              ];
                            case "intermidiateLux":
                              return [
                                <MenuItem
                                  key="lux"
                                  onClick={() => handleMenuItemClick("LUX")}
                                >
                                  LUX
                                </MenuItem>,
                              ];
                            case "intermidiateNoise":
                              return [
                                <MenuItem
                                  key="noise"
                                  onClick={() => handleMenuItemClick("dBA")}
                                >
                                  dBA
                                </MenuItem>,
                              ];
                            case "intermidiateFlow":
                              return [
                                <MenuItem
                                  key="flow"
                                  onClick={() => handleMenuItemClick("m3/h")}
                                >
                                  m3/h
                                </MenuItem>,
                              ];
                            case "intermidiateHeat":
                              return [
                                <MenuItem
                                  key="heat"
                                  onClick={() => handleMenuItemClick("mJ/h")}
                                >
                                  mJ/h
                                </MenuItem>,
                              ];
          ...
          >
            {
              // <HeatmapChart
              //   chartData={sampleHeatmapData.chartData}
              //   xAxis={sampleHeatmapData.xAxis}
              //   minValue={sampleHeatmapData.minValue}
              //   maxValue={sampleHeatmapData.maxValue}
              //   criticalValue={sampleHeatmapData.criticalValue}
              // />
